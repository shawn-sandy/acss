...
| `label`           | `string`                                                             | —          | Optional label text. Always in the accessibility tree. Visual rendering controlled by `showLabel`.|
| `type`            | `'button' \| 'submit' \| 'reset'`                                    | `'button'` | Button type attribute. Required.                                                             |
| `showLabel`       | `boolean`                                                            | `false`    | `false`: label is `sr-only` (always visually hidden). `true`: label visible; CSS breakpoint hides it below `48rem`.|
...
Visual rendering depends on the `showLabel` prop:

| `showLabel` | Result |
| ----------- | ------ |
| `false` (default) | Label receives the `sr-only` class — visually hidden on **all** viewport sizes. Screen readers always announce it. |
| `true` | Label is fully visible. The `$icon-label-bp` CSS media query controls responsive hiding: hidden below `48rem` (768px), visible above. |

```tsx
// Always sr-only — icon-only appearance at every breakpoint
<IconButton type="button" aria-label="Settings" icon={<GearIcon />} label="Settings" />
...
The responsive breakpoint is a SCSS variable (not a CSS custom property) because
media query conditions are evaluated at parse time and cannot reference runtime values:
...
Pass `showLabel` to make the label visually rendered. Use `variant="outline"`
(or any non-`icon` variant) to restore padding alongside the label. The default
`variant="icon"` sets `padding: 0`, which collapses the layout around a label.
...
Without `showLabel`, the label is sr-only regardless of viewport width — useful
when you want screen reader announcement but an icon-only visual:

```tsx
// sr-only label: screen reader says "Settings", sighted users see icon only
<IconButton type="button" aria-label="Settings" icon={<SettingsIcon />} label="Settings" />
```
