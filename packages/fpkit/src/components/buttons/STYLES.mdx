...
- **Size variants** - Extra small (xs) through large (lg) with fluid
  typography
...
```html
<button style="--btn-hover-transform: scale(1.05); --btn-hover-filter: brightness(0.9)">
  Hover Me
</button>
```
...
```html
<button style="--btn-focus-outline: 3px solid blue; --btn-focus-outline-offset: 2px">
  Focus Me
</button>
```
...
<button
  type="button"
  style="--btn-bg: #D32F2F; --btn-color: white"
>
  Delete
</button>
```
...
```html
<button style="--btn-width: 100%" type="submit">
  Continue
</button>
```
