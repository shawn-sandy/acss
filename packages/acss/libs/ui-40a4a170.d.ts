 ...
 * ```
 */
type PolymorphicComponentProp<C extends React.ElementType, Props extends object = Record<string, never>> = React.PropsWithChildren<Props & AsProp<C>> & Omit<React.ComponentPropsWithoutRef<C>, PropsToOmit<C, Props>>;
/**
 ...
 * ```
 */
type PolymorphicComponentPropWithRef<C extends React.ElementType, Props extends object = Record<string, never>> = PolymorphicComponentProp<C, Props> & {
 ...
 * ```
 */
type UIComponent = (<C extends React.ElementType = "div">(props: UIProps<C>) => React.ReactElement | null) & {
    displayName?: string;
};
/**
