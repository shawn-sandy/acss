...
### Error: "Already on a release branch"

**Symptom:**
```
❌ Already on a release branch. Switch to main/master first.
   Current branch: release/v0.6.1
   To switch: git checkout main
```

**Cause:** You're currently on a release branch from a previous publish run
...
2. **Delete stale release branch if needed:**
   ```bash
   # Check what's on the release branch
   git log release/v0.6.1

   # If it's already merged, delete it
   git branch -d release/v0.6.1

   # If it's not merged but you want to delete anyway
   git branch -D release/v0.6.1
   ```
...
### Error: Merge Conflict After Publish

**Symptom:**
```
❌ Merge failed: error: Your local changes to the following files would be overwritten by merge:
        package.json
⚠️  Release branch 'release/v0.6.2' preserved for manual intervention
```

**Impact:** ⚠️  CRITICAL - Packages published but merge back to main failed

**What happened:**
1. ✅ Packages successfully published to npm
2. ✅ Git tags created on release branch
3. ❌ Failed to merge release branch into main
...
3. **Manually merge with conflict resolution:**
   ```bash
   git merge release/v0.6.2
   ```
...
6. **Cleanup release branch:**
   ...
   git branch -d release/v0.6.2

   # Optional: Delete remote release branch
   # (Note: v0.3.0 preserves remote branches by default)
   git push origin --delete release/v0.6.2
   ```
...
3. **Or stash changes temporarily:**
   ```bash
   # Stash changes
   git stash
   ...
   # Restore stashed changes after publish
   git stash pop
   ```
...
**Symptom:**
```
⚠️  Warning: Branch not up to date with remote. Consider pulling latest changes.
```
...
### Scenario: Publish Failed After Release Branch Created

**What happened:**
1. ✅ Release branch created: `release/v0.6.2`
2. ❌ Publish failed (OTP invalid, network error, etc.)

**Cleanup offered:**
```
⚠️  Publish failed. Release branch 'release/v0.6.2' was created.
Delete release branch and return to original? (yes/no):
```

**Options:**

**Option 1: Accept cleanup (recommended for OTP failures)**
```
Delete release branch and return to original? (yes/no): yes
✅ Cleaned up release branch: release/v0.6.2
```

**Option 2: Keep release branch for investigation**
```
Delete release branch and return to original? (yes/no): no
Release branch 'release/v0.6.2' preserved for investigation.
```
...
# Force delete release branch (has unpublished changes)
git branch -D release/v0.6.2

# Delete remote if it was pushed
git push origin --delete release/v0.6.2
```
